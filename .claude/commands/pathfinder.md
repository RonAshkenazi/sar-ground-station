---
name: pathfinder
description: Map a codebase into feature-grouped flowcharts, identify duplicated concerns across features, and propose a unified architecture. Use when asked to audit architecture before a refactor or unify duplicated systems.
---
...
For each duplicated concern that is NOT legitimate specialization:
1. Propose the simplest unified design
...
End the document with ONE combined Mermaid flowchart showing the proposed unified system.
...
- **Simplest unification wins** — prefer deletion over abstraction
- **Specialization is not duplication** — two components serving different trust models are legitimate even if code looks similar
- **Handoff, don't implement** — Pathfinder ends at plan prompts; `/make-plan` and `/do` take it from there

## Failure Modes to Prevent

- Drawing flowcharts from memory instead of source
- Proposing unification of legitimately specialized components
- Handoff prompts that lack concrete call sites
- Skipping Phase 0 boundary review
